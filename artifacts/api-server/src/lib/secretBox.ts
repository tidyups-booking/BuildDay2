...
function encryptionKey(info: string): Buffer {
  ...
  if (!root) {
    throw new Error(
      "SESSION_SECRET is required to encrypt stored credentials",
    );
  }
...
export function decryptSecret(payload: string | null, info: string): string | null {
...
export const decryptQuoKey = (payload: string | null) => decryptSecret(payload, QUO_INFO);

/** Jobber OAuth token helpers */
const JOBBER_INFO = "bookmycleaning:jobber-token:v1";
export const encryptJobberToken = (token: string) => encryptSecret(token, JOBBER_INFO);
export const decryptJobberToken = (payload: string | null) => decryptSecret(payload, JOBBER_INFO);

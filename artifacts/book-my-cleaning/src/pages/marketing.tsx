...
import { CheckCircle2, Phone, Calendar, Zap, ArrowRight, Play, Mic, Star } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-brand-pink/20 selection:text-white">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 bg-background/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-serif font-extrabold text-xl tracking-tight">Tidyups</span>
          </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div className="max-w-2xl relative z-10">
            ...
            <p className="text-lg text-muted-foreground mb-10 leading-relaxed max-w-xl">
              Professional residential & commercial cleaning across Edmonton — from deep cleans to move-outs. Trusted, background-checked pros who show up on time. <span className="text-white font-medium">Leave the mess to us.</span>
            </p>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
              <Link href="/sign-up">
                <Button size="lg" className="h-14 px-8 rounded-full text-base font-bold gap-2 w-full sm:w-auto brand-gradient border-0 hover:opacity-90 shadow-[0_0_30px_rgba(236,72,153,0.3)] text-white">
            ...
            <div className="flex flex-wrap items-center gap-6 mt-12 text-xs font-medium text-muted-foreground">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-brand-pink" /> Insured & Bonded
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-brand-pink" /> Eco-Friendly
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-brand-pink" /> Satisfaction Guaranteed
              </div>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="relative rounded-3xl border border-white/10 bg-[#120e17] shadow-2xl p-2 z-10 transform lg:rotate-[-2deg] transition-transform hover:rotate-0 duration-500 overflow-hidden">
              ...
              <div className="bg-[#0d0a0f] rounded-2xl border border-white/5 overflow-hidden relative">
                ...
                <div className="p-4 border-b border-white/5 bg-[#120e17] flex items-center gap-3 relative z-10">
                  ...
                  <div>
                    <div className="font-bold text-sm text-white">Live Call Transcript</div>
                ...
                <div className="p-4 space-y-4 max-h-[300px] overflow-hidden relative z-10 bg-[#0d0a0f]">
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-white/10 shrink-0 flex items-center justify-center text-xs font-bold text-white">S</div>
                    <div className="bg-[#1a1523] border border-white/5 rounded-2xl rounded-tl-none px-4 py-3 text-sm text-white/90 shadow-sm">
                      Hi, I need a deep clean for a 3-bedroom house next Tuesday.
                    </div>
                  </div>
                  <div className="flex gap-3 flex-row-reverse">
                    <div className="w-8 h-8 rounded-full brand-gradient shrink-0 flex items-center justify-center text-xs font-bold text-white">AI</div>
                    <div className="brand-gradient text-white rounded-2xl rounded-tr-none px-4 py-3 text-sm shadow-sm">
                      I can help with that! Our deep cleaning for a 3-bedroom home usually takes 4-5 hours. Tuesday morning is available. May I get your address?
                    </div>
                  </div>
            ...
            <div className="absolute -bottom-6 -left-6 bg-[#1a1523] rounded-2xl border border-white/10 shadow-2xl p-4 flex items-center gap-4 z-20 animate-in slide-in-from-bottom-4 duration-700 delay-300">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">Happy Edmonton clients</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full bg-[#1a1523] border-y border-white/5 py-4 overflow-hidden flex relative z-10">
        <div className="flex animate-[marquee_20s_linear_infinite] whitespace-nowrap items-center gap-12 px-6">
          {["OFFICE CLEANING", "RECURRING PLANS", "POST-CONSTRUCTION", "INSURED & BONDED", "DEEP CLEANING", "MOVE-OUT", "AIRBNB TURNOVER", "ECO-FRIENDLY"].map((text, i) => (
            <div key={i} className="flex items-center gap-4 text-xs font-bold tracking-widest text-muted-foreground">
          ...
          {["OFFICE CLEANING", "RECURRING PLANS", "POST-CONSTRUCTION", "INSURED & BONDED", "DEEP CLEANING", "MOVE-OUT", "AIRBNB TURNOVER", "ECO-FRIENDLY"].map((text, i) => (
            <div key={`dup-${i}`} className="flex items-center gap-4 text-xs font-bold tracking-widest text-muted-foreground">
              <Star className="w-3 h-3 text-brand-pink" />
              {text}
            </div>
          ))}
      ...
      <section className="py-24 bg-background relative z-10">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-20">
            <div className="text-brand-pink text-xs font-bold uppercase tracking-widest mb-4">Our Services</div>
            <h2 className="font-serif text-3xl md:text-5xl font-extrabold text-white mb-6">Cleaning solutions for every need</h2>
            <p className="text-muted-foreground text-lg">We've made getting started as easy as filling out a checklist. No coding, no complex prompt engineering.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="bg-[#120e17] rounded-3xl p-8 border border-white/5 shadow-xl hover:border-white/10 transition-colors relative overflow-hidden group">
                ...
                <h3 className="font-serif text-xl font-extrabold text-white mb-3">{item.title}</h3>
      ...
      <section className="py-24 px-6 bg-[#1a1523] border-t border-white/5 relative overflow-hidden">
        <div className="absolute inset-0 opacity-20 mix-blend-overlay" style={{ backgroundImage: "radial-gradient(circle at center, rgba(255,255,255,0.1) 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
        
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <h2 className="font-serif text-4xl md:text-6xl font-extrabold mb-8 text-white">Stop letting revenue ring out.</h2>
          <p className="text-muted-foreground text-lg md:text-xl mb-10 max-w-2xl mx-auto">
            Join the cleaning businesses that are booking more jobs while scrubbing less floors. Your new best employee never sleeps.
          </p>
          <Link href="/sign-up">
            <Button size="lg" className="h-16 px-10 rounded-full text-lg font-bold brand-gradient text-white border-0 shadow-[0_0_40px_rgba(236,72,153,0.4)] hover:scale-105 transition-transform">
      ...
      <footer className="py-12 border-t border-white/5 bg-[#0a080c] text-center text-muted-foreground text-sm">
        <div className="flex items-center justify-center gap-3 mb-6">
          <img src="/logo.svg" alt="Book My Cleaning" className="w-6 h-6 opacity-80" />
          <span className="font-serif font-extrabold tracking-tight text-white/80">Tidyups</span>
        </div>
        <p>&copy; {new Date().getFullYear()} Tidyups Cleaning Service Inc. All rights reserved.</p>
      </footer>
    </div>
  );
}
